...
// Safe replacement for RegistrantRow
const split1 = code.split('function RegistrantRow({');
const split2 = split1[1].split('function DuplicatesPanel({');
code = split1[0] + newRow + "function DuplicatesPanel({" + split2[1];

// Safe replacement for the map wrapper
const mapStart = '<div className="border border-ink-800 rounded-xl overflow-hidden bg-ink-900 shadow-sm">';
const mapEnd = '        )}\n      </div>';
const startIndex = code.indexOf(mapStart);
if (startIndex !== -1) {
    const endIndex = code.indexOf(mapEnd, startIndex) + mapEnd.length;
    if (endIndex !== -1) {
