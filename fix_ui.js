...
const newRow = `function RegistrantRow({
  registrant: r,
  pickupPoints,
  role,
  onStatusChange,
...
const newMap = `      <div className="border border-line rounded-xl overflow-x-auto bg-surface shadow-md">
...
cp.execSync('git checkout origin/main -- components/AdminDashboard.tsx');
code = fs.readFileSync('components/AdminDashboard.tsx', 'utf8');

// 2. Safe string replacement without Regex edge cases
const r_start = code.indexOf('function RegistrantRow({');
const r_end = code.indexOf('function DuplicatesPanel({', r_start);
if (r_start !== -1 && r_end !== -1) {
    code = code.substring(0, r_start) + newRow + '\\n\\n' + code.substring(r_end);
}

const map_start = '<div className="border border-ink-800 rounded-xl overflow-hidden bg-ink-900 shadow-sm">';
const map_end = 'No registrants found.</p>\\n        )}\\n      </div>'; // Let's use precise split
// Actually let's just do a simpler replace of the wrapper:
const mapIdx = code.indexOf(map_start);
if (mapIdx !== -1) {
    const endIdx = code.indexOf('</div>', code.indexOf(')}', mapIdx)) + 6;
    code = code.substring(0, mapIdx) + newMap + code.substring(endIdx);
}

// Global color fixes
code = code.replace(/bg-ink-950/g, 'bg-surface')
code = code.replace(/bg-ink-900/g, 'bg-surface-2')
code = code.replace(/border-ink-800/g, 'border-line')
code = code.replace(/text-cream-50/g, 'text-ink-900')
code = code.replace(/text-cream-200/g, 'text-ink-soft')
code = code.replace(/text-cream-100/g, 'text-ink')
