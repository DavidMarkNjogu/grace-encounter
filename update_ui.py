  ...
  return (
    <>
      <tr className="border-b border-line hover:bg-surface-2 transition-colors group">
        <td className="px-4 py-3 text-sm text-ink-soft whitespace-nowrap">{index}</td>
        <td className="px-4 py-3 text-sm text-ink whitespace-nowrap">{r.list_number !== null ? # : "-"}</td>
      ...
      {showNote && (
        <tr className="bg-bg border-b border-line">
          <td colSpan={8} className="px-4 py-3 pl-8">
...
# Slice the code manually instead of relying on regex which might fail due to whitespace
split1 = code.split('function RegistrantRow({')
split2 = split1[1].split('export default function AdminDashboard({')

code = split1[0] + new_row + "\n\nexport default function AdminDashboard({" + split2[1]

# Now replace the paginatedRegistrants.map wrapper
map_start = '        <div className="border border-ink-800 rounded-xl overflow-hidden bg-ink-900 shadow-sm">'
map_end = '        )}'
# Actually let's just find the exact block:
import re
new_mapping_block = r"""      <div className="border border-line rounded-xl overflow-x-auto bg-surface shadow-md">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-line bg-surface-2">
              <th className="px-4 py-3 text-[11px] font-semibold text-ink-soft uppercase tracking-wider whitespace-nowrap">No.</th>
          ...
          <tbody>
            ...
            {paginatedRegistrants.length === 0 && (
              <tr>
                <td colSpan={8} className="p-8 text-center text-sm text-ink-soft">No registrants found.</td>
              </tr>
            )}
          </tbody>
        </table>
...
start_idx = code.find('<div className="border border-ink-800')
if start_idx == -1:
    print("Could not find table wrapper!")
    sys.exit(1)
end_str = 'No registrants found.</div>\n        )}\n      </div>'
end_idx = code.find(end_str) + len(end_str)

code = code[:start_idx] + new_mapping_block + code[end_idx:]
